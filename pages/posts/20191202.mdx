...
以前15erの方が、[CPU実験でTravis CIを使ってみた](http://lv4.hateblo.jp/entry/2015/12/15/104931)という記事を書かれており、
...
その使用目的については、AWS の記事がわかりやすいので、貼っておきます: https://aws.amazon.com/jp/devops/continuous-integration/   
...
https://github.com/cpuex-19-6/rytr_test
...
![pic1](/static/20191202/pic1.jpg "テストが回る様子")
...
さらに、CIは、デフォルトではリポジトリにpushが起こる時のみにテストが回る仕組みですが、
...
レイトレプログラムは、まずそれ自体が動かすのに時間がかかります。128 × 128 サイズならまだましですが、
512 × 512 サイズとなると10分では済まないと思います。  
さらに、（これは班によって異なるかもしれませんが、）コンパイラとシミュレータは独立に開発されているものでしょう。  
...
それが逆に新たなバグを埋め込むことが起こり得ます。そもそも CI とは、それを防止するツールです。  
私の使用例では、個々の（コンパイラなどの）リポジトリとCIが直接紐づいているわけではないので、どのコミットが原因か
一意に定まるわけではないのですが、それでも1時間に1回テストを回しているため、原因特定はしやすいです。
...
15erの方の記事を読むと、Travis CI で OCaml を使えるようにするため、手間取ったようです。 
（15er の方は Python でテストスクリプトを書いている（！）などと私の場合より手が込んでいるという事情もあると思います）   
しかし、今回 Circle CI を使ってみて、導入コストは全く感じませんでした。CI は気軽に導入できます。
Circle CI は Docker をベースにしているのも手軽さの一因かと思います。

<br/>

**楽しい**

Slack とインテグレーションさせることもできるそうです（私はやっていませんが、Circle CI でもできるはずです）。

<blockquote className="twitter-tweet" data-lang="ja"><p lang="ja" dir="ltr">CPU実験、コンパイラ/アセンブラ/シミュレータの結合テストをTravis CIで実現したので、Githubにpushしてテスト結果がSlackに流れるのがめちゃくちゃ楽しい<a href="https://t.co/ZlcR99hQPl">https://t.co/ZlcR99hQPl</a></p>&mdash; Han Bao (@levelfour_) <a href="https://twitter.com/levelfour_/status/656043900017086464?ref_src=twsrc%5Etfw">2015年10月19日</a></blockquote>
