...
これまでこの `hellorusk.net` は Netlify で配信していたが、ブログシステムの改築に合わせて、Zeit の [Now](https://zeit.co) に乗り換えることに決めた。  
...
で取得したドメインのサブドメインを Netlify でホスティングする、など）場合、CNAME レコードを用いている。  
